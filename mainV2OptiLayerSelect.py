...
async def server(websocket):
    ...
    async for message in websocket:
        ...
        if "filestart" in message:
            ...
            #print(tempArray)
            layerList=[]
            layerList.append('All_Layer')
            for idm in range (len(tempArray)):
                if '"' in tempArray[idm]:
                    layerList.append("/")
                    layerList.append(tempArray[idm][1:-1])

                    #print(tempArray[idm])
            print(layerList)
            await websocket.send(layerList)
        ...
        if "BULLSEYEdisable" in message:
            SerialConnect("G28 \n")
            SerialConnect("G92 X0 Y0 \n")
        if "LAYERSELECTED" in message:
            selectedArray=[]
            #message=message.replace("LAYERSELECTED/","")
            layerSelect=message.split("/")
            print(tempArray)
            tempArrayJoin='\n'.join(tempArray)
            layeringArray=tempArrayJoin.split("]")
            for idp in range(len(layerSelect)):
                for ido in range(len(layeringArray)):
                    if layerSelect[idp] in layeringArray[ido]:
                        selectedArray.append(layeringArray[ido])
            print("new selected is")
            selectedArray='\n'.join(selectedArray)
            print(selectedArray)
            ...
            xnow = "0"
            ynow = "0"
            tempArray1=selectedArray

            print("length of temp array is {}".format(len(tempArray1)))
            for idx in range(len(tempArray1)):
                stateCheck = ""
                if "G01" in tempArray1[idx]:
                    coords = re.findall(r'-?\d+(?:\.\d+)?', tempArray1[idx])
                    # xval=extract_substring(tempArray1[idx],"X","Y")
                    # yval=extract_substring(tempArray1[idx],"Y","\d")
                    xval = coords[1]
                    yval = coords[2]
                    layerarray1 += xnow + " " + ynow + " " + xval + " " + yval + " " + "1 1,"
                    xnow = xval
                    ynow = yval
                    stateCheck = "G01"
                if "G00" in tempArray1[idx]:
                    coords = re.findall(r'-?\d+(?:\.\d+)?', tempArray1[idx])
                    # xval = extract_substring(tempArray1[idx],"X","\d")
                    # yval = extract_substring(tempArray1[idx],"Y"," ")
                    xval = coords[1]
                    yval = coords[2]
                    layerarray1 += xnow + " " + ynow + " " + xval + " " + yval + " " + "2 2,"
                    xnow = xval
                    ynow = yval
                    stateCheck = "G00"
                if is_different_from_previous_input(stateCheck) == True:
                    if stateCheck == "G01":
                        CommandList1.append("M8")
                        CommandList1.append("G4 P1")
                    if stateCheck == "G00":
                        CommandList1.append("M9")
                        CommandList1.append("G4 P1")
                CommandList1.append(tempArray1[idx])

            gcodeArray = CommandList1
            for idv in range(len(gcodeArray)):
                gcodeArray[idv] = gcodeArray[idv].strip()
            gcodeArray = [line for line in gcodeArray if line.strip() != ""]
            print("GCODE ARRAY>>>>>>>>>>>>>>>>>>>")
            ...
            # print(message)
            # print(layerarray1)
            await websocket.send("LAYERVIEW" + layerarray1)
            print(" layer array is {}".format(layerarray1) )

            PosMatrix = layerarray1.split(",")
            # print("POSMATRIX>>")
            # print(PosMatrix)
            wMat = 3
            hMat = len(PosMatrix)
