...
def PlotBegin():
    ...
    while True:
        ...
        for idg in range(len(gcodeArray)):
            ...
            GlobalPercentage = int((idg / len(gcodeArray)) * 100)
            print("percentage is {}".format(GlobalPercentage))
            #await websocket.send("PERCENTAGE" + str(progressPercentage))
            ...
            if "Bf:" in state:
                stateOrder = extract_substring(state, "Bf:", ",")
                stateOrder = int(stateOrder)
                print("internal buffer is {}".format(stateOrder))
                if stateOrder < 35:
                    while True:
                        while websocketRTV.threadStatus == 0:
                            pass
                        sleep(0.5)
                        checkBuffer = SerialConnect("?\n")
                        print(checkBuffer)
                        if "MPos:" in checkBuffer:
                            mposition = re.findall(r'-?\d+(?:\.\d+)?', checkBuffer)
                            Mposx = mposition[0]
                            Mposy = mposition[1]
                            print("Mposx>>{}".format(Mposx))
                            print("Mposy>>{}".format(Mposy))
                        if "Bf:" in checkBuffer:
                            currentOrder = extract_substring(checkBuffer, "Bf:", ",")
